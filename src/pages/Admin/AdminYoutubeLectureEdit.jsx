...
import { addLectureToYoutubeCourse, editSpecificYoutubeLecture, getSpecificYoutubeLecture } from '../../redux/actions/youtube'
import LoadingComponent from '../../components/Loading'

const AdminYoutubeLectureEdit = () => {
    ...
    const { loading, error, message, lecture } = useSelector(state => state.youtube);
    ...
    useEffect(() => {
        dispatch(getSpecificYoutubeLecture(id, lectureid));
    },[]);
    ...
    useEffect(() => {
        if(lecture){
            ...
            setUrl(lecture.url);
            convertPosterToBase64(lecture.thumbnail.url);
        }
    }, [lecture]);
    ...
    const editLectureDetails = async (e, id, title, description, url, image) => {
        e.preventDefault();
        ...
        await dispatch(editSpecificYoutubeLecture(formData, id, lectureid));
    }
            ...
            <MainWrapper pt={24} pb={12}>
                <VStack gap={4}>
                    <HStack justifyContent={'flex-start'}>
                        <Breadcrumb spacing='8px' fontWeight={'normal'} fontSize={'xs'} separator={<FaAngleRight color='gray.500' />}>
                            ...
                            <BreadcrumbItem isCurrentPage>
                                <BreadcrumbLink color={'purple'} href='#'>Edit Lecture</BreadcrumbLink>
                            </BreadcrumbItem>
                        </Breadcrumb>
                    </HStack>

                    <VStack gap={0} width={'full'}>
                        <Heading mt={2} textAlign={'center'} fontFamily={'Young Serif'} fontSize={['2xl', '2xl', '3xl', '4xl']}>Edit Lecture</Heading>
                        <Text mt={['1', '1', '2', '2']} fontSize={['sm', 'sm', 'md', 'md']} width={['80%', '', '', '']} textAlign={'center'} >Hey Boss👋, edit the lecture of the course the way you want.</Text>
                    </VStack>
                    ...
                    <VStack gap={2} mt={4} width={['95%', '95%', '60%', '70%']}>
                        ...
                        <Button width={'full'} onClick={e => editLectureDetails(e, id, title, description, url, image)} isDisabled={!title || !description || !image || !imagePrev} colorScheme='purple' size={['sm', 'sm', 'md', 'md']} mt={2} gap={2}>Make Changes <MdCheckCircle /> </Button>
                    </VStack>
...
export default AdminYoutubeLectureEdit
